...
class MBartFlashAttention2(MBartAttention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "MBartFlashAttention2 attention does not support `output_attentions=True` or `layer_head_mask is not None`. "
...
class MBartSdpaAttention(MBartAttention):
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "MBartSdpaAttention attention does not support `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
