...
class UniSpeechFlashAttention2(UniSpeechAttention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "UniSpeechFlashAttention2 attention does not support `output_attentions=True` or `layer_head_mask is not None`. "
...
class UniSpeechSdpaAttention(UniSpeechAttention):
    ...
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "UniSpeechSdpaAttention attention does not support `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
