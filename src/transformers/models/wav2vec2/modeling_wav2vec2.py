...
class Wav2Vec2FlashAttention2(Wav2Vec2Attention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "Wav2Vec2FlashAttention2 attention does not support `output_attentions=True` or `layer_head_mask is not None`. "
...
class Wav2Vec2SdpaAttention(Wav2Vec2Attention):
    ...
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "Wav2Vec2SdpaAttention attention does not support `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
