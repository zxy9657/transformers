...
class WhisperFlashAttention2(WhisperAttention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "WhisperFlashAttention2 attention does not support `output_attentions=True` or `layer_head_mask is not None`. "
...
class WhisperSdpaAttention(WhisperAttention):
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "WhisperSdpaAttention attention does not support `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
