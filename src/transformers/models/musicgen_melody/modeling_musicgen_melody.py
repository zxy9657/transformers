...
class MusicgenMelodyFlashAttention2(MusicgenMelodyAttention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "MusicgenMelodyFlashAttention2 attention does not support `output_attentions=True` or `layer_head_mask is not None`. "
...
class MusicgenMelodySdpaAttention(MusicgenMelodyAttention):
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "MusicgenMelodySdpaAttention attention does not support `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
