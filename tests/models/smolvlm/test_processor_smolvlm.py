...
class SmolVLMProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    ...
    def test_process_interleaved_images_prompts_no_image_splitting(self):
        ...
        self.assertEqual(np.array(inputs['pixel_values']).shape, (3, 3, 512, 512))
        self.assertEqual(np.array(inputs['pixel_attention_mask']).shape, (3, 512, 512))
        # fmt: on
    ...
    def test_process_interleaved_images_prompts_image_splitting(self):
        ...
        # Test that a single image is processed correctly
        inputs = processor(images=self.image1)
        self.assertEqual(np.array(inputs["pixel_values"]).shape, (13, 3, 512, 512))
        self.assertEqual(np.array(inputs["pixel_attention_mask"]).shape, (13, 512, 512))
        # fmt: on
        self.maxDiff = None
        ...
        self.assertEqual(np.array(inputs['pixel_values']).shape, (43, 3, 512, 512))
        self.assertEqual(np.array(inputs['pixel_attention_mask']).shape, (43, 512, 512))
        # fmt: on
    ...
    def test_non_nested_images_with_batched_text(self):
        ...
        self.assertEqual(np.array(inputs["pixel_values"]).shape, (4, 3, 512, 512))
        self.assertEqual(np.array(inputs["pixel_attention_mask"]).shape, (4, 512, 512))
    ...
    def test_chat_template_video(self):
        ...
        self.assertEqual(len(out_dict_with_video[self.videos_input_name]), 10)
    ...
    # Override as SmolVLMProcessor returns flattened images/patches
    @require_av
